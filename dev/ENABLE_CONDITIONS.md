...
| `toggle-auto-no-action` | **corp** (rendered in `corp-run-div` only) ∧ run ∧ encounter-count ≤ 1 ∧ ¬success | `toggle-auto-no-action!`: side · **✨ run (`:server` present) ∧ encounter-count ≤ 1 ∧ ¬success**; engine toggle is now a no-op with no run **and for any side but the Corp** | ai-actions-test `auto-pass-refused-outside-a-run`, `auto-pass-mirrors-the-web-ui-button`; runs-test `auto-no-action-toggle-outside-a-run-is-a-no-op`, `auto-no-action-toggle-is-corp-only` | ✨ (#221: unguarded, it CREATED a phantom `:run` that refused every click action for the rest of the game) · ✅ corp-only since #224 (the engine bound `side` as `_`, so a Runner `:game/action` flipped the Corp's flag and pressed continue for the Corp) |
